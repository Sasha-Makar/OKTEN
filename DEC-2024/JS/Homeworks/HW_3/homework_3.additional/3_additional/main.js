...
let count = 50;
for (let i = 0; someArr.length < count; i++) {
...
someArr1 = [];
count = 50;
while (someArr1.length < count) {
    let randomNum = Math.floor(Math.random() * 100) + 1;
    if (randomNum % 2 !== 0) {
        someArr1.push(randomNum);
    }
}
...
for (let i = 0; newArr.length < counter; i++) {
    let randomNum = Math.floor(Math.random() * 100) + 1;
...
for (let i = 0; newArr2.length < counter; i++) {
    let randomNum = Math.floor(Math.random() * (732 - 8)) + 8;
    newArr2.push(randomNum);
}
...
for (let i = 2; i < newArr2.length; i += 3) {
    let item = newArr2[i];
    console.log(item);
}
...
// 3. Вивести за допомогою console.log кожен третій елемен
// тільки якщо цей елемент є парним.
...
// 6. Є масив з числами [100,250,50,168,120,345,188], Які
// характеризують вартість окремої покупки. Обрахувати середній чек.


let checkArr = [100, 250, 50, 168, 120, 345, 188];
let sum = 0;
for (const number of checkArr) {
    sum += number
}
console.log(sum);
let averageSum = sum / checkArr.length;
console.log(parseInt(averageSum.toFixed()));
...
let diffValueArr = [25, 'hello', 42, 'world', 7, true, 13, 'test', 99, false];
console.log(diffValueArr);
let someArray = [];

for (const item of diffValueArr) {
...
// З'єднати в один об'єкт користувача та місто з
...
// - Взяти масив з 10 чисел або створити його.
// Створити 2й порожній масив. За допомогою
// будь-якого циклу скопіювати значення одного
// масиву в інший.
